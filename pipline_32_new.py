...
XA=np.load('/home/u3749/code/64_rescale_all_X.npy')
YA=np.load('/home/u3749/code/64_rescale_all_Y.npy')
...
XAC = [transform_image(img, rescaled_dim) for img in XA]
XAC = np.array(XAC).squeeze()

X,Y=shuffle(XAC,YA)

np.save('/home/u3749/code/32_rescale_all_X.npy',X)
np.save('/home/u3749/code/32_rescale_all_Y.npy',Y)


X=np.load('/home/u3749/code/32_rescale_all_X.npy')
Y=np.load('/home/u3749/code/32_rescale_all_Y.npy')
X,Y=shuffle(X,Y)
...
model.fit(X, Y, n_epoch=500, shuffle=True, validation_set=0.25,show_metric=True, batch_size=96, run_id='cvix_cnn')
