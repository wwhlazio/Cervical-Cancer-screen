...
#downsample the image data to 32*32
...
#implement for the folowing 3 kinds of type separately.3 typ2s can be run separately.
t=all_cervix_images['type'].unique()[0]
t=all_cervix_images['type'].unique()[1]
t=all_cervix_images['type'].unique()[2]

#load image data to images
...
for type 1:
#tow images can not be loaded, delte the image name.
...
#delete image files needed to be deleted
...
delete the image files asked to be deleted
...
#load  the train data. the deleted two variables are huge
del images
del all_images
...
#combine the train file and delete file
...
#exchange the labels of the files as requested
...
#load the stage 2 final test data
